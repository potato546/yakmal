...
- ingredients: 주성분·원료·전성분 정보가 있을 때만. [{"name_ko":..., "name_tr":..., "amount":..., "role_ko":..., "role_tr":...}].
  name_tr은 그 언어권에서 실제로 쓰이는 정식 화학·의약·식품 성분명으로 번역할 것 (예: 아세트아미노펜→中文:对乙酰氨基酚/日本語:アセトアミノフェン,
  산화마그네슘→中文:氧化镁/日本語:酸化マグネシウム, 비타민 B1→中文:维生素B1/日本語:ビタミンB1). 한국어 발음을 소리 나는 대로 옮기거나(음역)
  일반 명사로 뭉뚱그리지 말고, 그 나라 약전·식품 라벨에 실제로 쓰이는 정확한 용어를 쓸 것. 확신이 없으면 化学式(화학식)이나 영문 성분명을
  괄호로 병기. amount(함량)의 단위도 그 언어로 번역할 것 (예: "350밀리그램"→中文:"350毫克"/日本語:"350ミリグラム"/English:"350 mg").
  숫자는 그대로 두고 단위 명칭만 번역. name은 고객이 알아들을 이름. role은 1~2문장, 과장 없이 — 이 제품의 [효능효과] 원문 맥락에 맞게 설명할 것.
  같은 성분이 다른 제품·다른 질환에는 다르게 쓰이더라도(예: 산화마그네슘은 제산제로도 쓰이지만 이 제품이 종합영양제면 "마그네슘 보충으로
  근육·신경 기능 유지"처럼 이 제품 목적에 맞게), 이 제품과 무관한 다른 용도로 단정하지 말 것.
  의약품·건강기능식품: [주성분]/[원재료명] 목록에 있는 성분을 하나도 빠짐없이 전부 배열에 넣을 것. 절대 일부만 골라 생략하지 말 것 —
  약사와 고객이 성분표 전체를 확인해야 하므로 누락은 정확성 문제가 됨. 부형제·향료처럼 원문에 함량 없이 나열만 된 것도 이름만 포함.
  화장품(기능성·일반): 전성분이 길면 미백·주름개선·보습 등 핵심 역할이 있는 3~5개만 선별.
  원문에 성분 목록 자체가 전혀 없을 때만 빈 배열. 반드시 객체 배열로.
...
(a) 제형+효능 4~5개: 반드시 첫 항목은 제형 단어 하나만 (예: "크림이에요", "정제예요", "캡슐이에요", "시럽이에요", "패치예요",
    ...
    성분 1개 포함 (예: "홍삼 성분이에요"). 성분명은 그 언어권 정식 화학·의약 용어로 (음역 금지, ingredients와 동일 규칙).
